...
○ 프로그램에서 발생하는 오류(Error)는
   ...
   ① 잘못된 문법을 사용하거나 변수 등을 정의하지 않은 상태에서 사용함으로써
      컴파일 단계에서 발생하는 문법적 오류(Syntax Error)
...
※ 정리해 놓은 문법~!!! (→ Exception 클래스)
   ...
   - 자바에서의 모든 예외 클래스는 Throwable 클래스나
     Throwable 클래스의 하위 클래스를 상속받아 사용한다. → 던질 수 있는 것들이다.
   ...
   - Throwable 클래스에서 파생된 클래스
	 ...
	 ① Exception 클래스
	    Exception 예외 클래스는 일반적으로 프로그래머에 의해 복원 될 수 있는 예외상황으로
...
※ 예외(Exception)의 종류
   ...
   - checked exception
     메소드 내에서 예외가 발생한 경우 
	 메소드를 정의할 때 『throws』문에 메소드 내에서 발생할 수 있는 예외들을 명시해주거나,
	 또는 그 예외를 『try~catch』해서 처리해주어야만 하는 예외이다.
	 컴파일러가 컴파일 하는 과정에서 『checked exception』이 『throws』되는가의 여부나 
...
{										   // ①  // ② 
	public static void main(String[] args) // Ⅹ  //throws IOException
	{
