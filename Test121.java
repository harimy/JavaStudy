...
○ 개요
   ...
   Sun(Oracle)사에서는 자바 프로그램을 효율적으로 작송할 수 있도록
   자바 표준 패키지를 제공하며,
...
○ import문
   ...
   프로그램에 포함시키는 과정이 필요한데,
   이 때 『import』문을 이용하여 패키지 또는 클래스를 프로그램에 포함시키게 된다.
   하지만, 『java.lang.*』패키지는 자바 프로그램에 기본적으로 import 되어있기 때문에
   ...
   ※ JDK 1.5 부터는 import 다음에 『static』키워드를 붙임으로써
      정적 패키지를 import 할 수 있는 기능을 제공하게 되었다.
	  만약 패키지를 정적으로 import 한 경우라면
	  모든 접근 가능한 메소드와 멤버 변수들은 
	  접두사(예 - ob, sc, br)를 붙이지 않고 사용하는 것이 가능하다.


○ 『java.lang.Object』클래스는 자바 클래스의 최상위 클래스로 
   자바 표준 패키지의 클래스나 사용자에 의해 작성된 모든 클래스는
   기본적으로 이 클래스로부터 상속받는다.
   따라서, 자바의 모든 클래스는 『java.lang.Object』클래스의 메소드를 가지고 있으며,
   바로 사용하는 것이 가능하다.
...
public class Test121 // extends Object
{
	...
	*/
	public static void main(String[] args)
	{
		...
		//--==>> 에러 발생(컴파일 에러)
		//-- 해당 클래스에 action() 이라는 메소드가 정의되어 있지 않기 때문에
