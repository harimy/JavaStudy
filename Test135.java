...
○ StringToken 클래스는
   ...
   여기에서 파싱(parsing)이란 하나의 문자열을 여러 문자열로 나누는 작업을 의미하게 되며,
...
○ 생성자
   ...
   → 주어진 문자열 str을 단어로 분리하는 객체를 생성하며
	  ...
	  만약 returnDelims 가 true라면, 공백(구분)문자도 분리된 유효한 데이터로 간주한다.
...
public class Test135
{
	public static void main(String[] args) throws IOException
	{
		...
		//   확인 결과만 반환할 뿐 헤드를 이동시키지 않는다.

		// 『nextTokens()』
		// : 다음 토큰을 얻어온다.
		//   다음 토큰을 얻어오며 헤드를 이동시키게 된다.
		//   만약 구분자를 바꾸었을 경우 다음 호출에도 바뀐 구분자를 이용하여 토큰을 분리한다.
...
/* [실행 결과]
파싱할 문자열 입력(컴마 구분): 사과,딸기,바나나,포도,복숭아,수박,파인애플
7
파싱된 문자열 :
