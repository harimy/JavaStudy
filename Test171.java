...
○ 객체 직렬화의 장점
   ...
   ※ Bean(빈)
      CS(Client와 Server) 구조적 모델에서 서버측 구조에 해당하며
...
○ Serializable 인터페이스
   ...
   - 객체 직렬화를 하기 위해 먼저 객체 직렬화가 가능하도록
     『java.io.Serializtion』 인터페이스를 구현해야 한다.
	 ...
	 ※ 멤버 변수가 static 으로 선언된 경우 (즉, 클래스 변수일 경우)
	    객체 직렬화의 대상에서 제외된다. → 인스턴스 변수들만 직렬화의 대상.

○ Object Stream

   - java.io.ObjectOutputStream (→ 바이트 기반의 스트림)
     ObjectOutputStream 클래스는 ObjectOutputStream 클래스에 의해
	 파일에 저장되어 있는 객체나 네트워크를 통해 전달된 객체의
	 직렬화를 해제하는 기능을 제공한다.
	 단, java.io.Serializable 인터페이스와
	 java.io.Externalizable(영속성있는) 인터페이스를 
	 지원해주는 객체에 대해서만 사용이 가능하다. 
	 (→ 직렬화나 영속성을 보장받은 객체에 대해서만 사용 가능)
...
public class Test171
{
	public static void main(String[] args) throws Exception
	{
		...
		if (!f0.getParentFile().exists())	//-- 디렉터리 구성이 존재하지 않는 상황이 맞다면...조건문 실행
		{
			// 디렉터리 경로를 구성하겠다. (만들겠다.)
		...
		//-- 파일 전용 출력 스트림(물줄기) 객체에 f0 라는 파일 객체를 띄움
		...
		oos.writeObject(h1);		//-- 생성된 스트림 객체에 내보낼 객체를 기록
		...
		if(f0.exists())	//-- f0 파일 객체가 존재한다면... 조건문 실행
		{
			FileInputStream fis = new FileInputStream(f0);
			ObjectInputStream ois = new ObjectInputStream(fis);

			// 읽어들인 객체(Object)를 다운 캐스팅(Hashtable 이라는 원래의 타입) 하여
			...
			// keys() → key 들을 Enumeration 형태로 반환
			//			 (※ Iterator 사용할 수 없음)
			...
			while (e.hasMoreElements())
			{
				...
				1 → 이희주
				*/
				// Map은 순서를 보장하지 않는다. 우연히 5,4,3,2,1 이렇게 출력된 것... 
			}
